//From AppCtrl.js 
//简要阐述：反编译第三方apk，在其清单文件中copy AhMyth的所有权限和service节点，然后正则匹配出清单文件中启动activity的activity的name，
//然后在其smali代码中添加startService以此关联启动来自AhMyth的后门代码。
//总结：1、由于签名不同，如果第三方apk做了签名校验，很容易出现签名校验不通过无法启动的情况，虽然可以通过pmshook签名解决 
// 2、未处理安卓6.0以上的权限动态申请，容易SecurityException crash
// 3、第三方库的重复依赖以及版本冲突（例如okhttp和okio）等等
// 4、整体来看，所有功能不算复杂
